 ...
 * GlobalLatencyTable
 * 
 * A clean SaaS Bento Box table presenting the normalized real global ping results.
 * Implementing a loading skeleton while it probes global servers.
 */
export default function GlobalLatencyTable({ data, isLoading }) {
    if (!isLoading && (!data || data.length === 0)) return null;

    const isDegraded = data && data.some(r => r.location === "Primary Server (Degraded Mode)");

    return (
        <div className="bg-white dark:bg-[#000000] rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-[#0B1120] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Global Latency Analysis</h3>
                {isDegraded && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-700/50">
                        ⚠️ High global network traffic. Showing local server latency.
                    </span>
                )}
            </div>
                            ...
                            <>
                                <tr>
                                    <td colSpan="7" className="px-6 py-4 text-center text-sm font-medium text-indigo-500 animate-pulse bg-indigo-50/50 dark:bg-indigo-900/10">
                                        Probing global network...
                                    </td>
                                </tr>
                                {Array.from({ length: 5 }).map((_, idx) => (
                                    <tr key={`skeleton-${idx}`} className={idx % 2 === 0 ? 'bg-white dark:bg-[#0B1120]' : 'bg-slate-50 dark:bg-[#000000]'}>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-slate-200 dark:bg-gray-800 rounded w-24 animate-pulse"></div></td>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-slate-200 dark:bg-gray-800 rounded w-20 animate-pulse"></div></td>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-5 bg-slate-300 dark:bg-gray-700 rounded-full w-16 animate-pulse"></div></td>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-slate-200 dark:bg-gray-800 rounded w-12 animate-pulse"></div></td>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-slate-200 dark:bg-gray-800 rounded w-12 animate-pulse"></div></td>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-slate-300 dark:bg-gray-700 rounded w-14 animate-pulse"></div></td>
                                        <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-slate-200 dark:bg-gray-800 rounded w-12 animate-pulse"></div></td>
                                    </tr>
                                ))}
                            </>
                            ...
                            data.map((row, idx) => (
                                <tr key={idx} className={idx % 2 === 0 ? 'bg-white dark:bg-[#0B1120]' : 'bg-slate-50 dark:bg-[#000000]'}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-white font-mono font-bold">
                                        {typeof row.avg === 'number' ? row.avg.toFixed(1) : row.avg}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-300 font-mono">
